...
export type LessonFilterOption = { id: string; firstName: string; lastName: string }
...
type Props = {
  lessons: LessonRow[]
  teachers: LessonFilterOption[]
  students: LessonFilterOption[]
  filters: {
    search: string
    teacherId: string
    studentId: string
...
export function LessonsTable({ lessons, teachers, students, filters }: Props) {
  ...
  const instruments = useMemo(() => {
    const set = new Set<string>(INSTRUMENT_OPTIONS)
    for (const l of lessons) set.add(l.instrument)
    ...
    <div className="flex flex-col gap-6 max-w-[1400px]">
      ...
      <div className="flex flex-col gap-3">
        <form onSubmit={handleSearchSubmit} className="flex flex-col sm:flex-row gap-3">
          ...
          <button
            type="submit"
            className="h-9 px-4 rounded-xl border border-border text-sm font-medium text-foreground hover:bg-accent transition-colors"
          >
            Search
          </button>
        </form>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <FilterSelect
            ...
            onChange={(v) => updateParams({ lessonType: v })}
            options={[
              { value: '', label: 'All types' },
              ...LESSON_TYPES.map((t) => ({ value: t, label: LESSON_TYPE_LABELS[t] })),
            ]}
          />
          <input
            type="date"
            ...
            className="h-9 rounded-xl border border-border bg-muted px-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-gold/40"
          />
          <input
            type="date"
            aria-label="To date"
          ...
          <FilterSelect
            ...
            options={[
              ...
              { value: 'teacher:desc', label: 'Teacher Z–A' },
          ...
          <table className="w-full min-w-[900px]">
            <thead>
              <tr className="border-b border-border bg-muted/40">
                {['Date', 'Time', 'Student', 'Teacher', 'Instrument', 'Type', 'Fee', 'Status', ''].map(
                  (h) => (
                    <th
                      key={h || 'actions'}
            ...
            <tbody className="divide-y divide-border">
              {lessons.length === 0 ? (
                ...
                lessons.map((lesson) => {
                  ...
                  return (
                    <tr key={lesson.id} className="hover:bg-accent/30 transition-colors">
                      ...
                      <td className="px-4 py-3.5 text-[13px] text-muted-foreground">{lesson.instrument}</td>
                      <td className="px-4 py-3.5 text-[13px] text-muted-foreground">
                        {LESSON_TYPE_LABELS[lesson.lessonType]}
                      </td>
                      <td className="px-4 py-3.5 text-[13px] text-foreground tabular-nums">
                        {formatFee(lesson.lessonFee)}
                      </td>
                        ...
                        {openMenuId === lesson.id && (
                          <div className="absolute right-4 top-10 z-20 w-40 rounded-xl border border-border bg-card shadow-lg py-1">
                            <Link
                              ...
                              className="flex items-center gap-2 px-3 py-2 text-[13px] text-foreground hover:bg-accent"
                            ...
                            <button
                              ...
                              className="flex w-full items-center gap-2 px-3 py-2 text-[13px] text-rose-500 hover:bg-rose-500/10 disabled:opacity-50"
