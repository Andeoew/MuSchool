import { Suspense } from 'react'
import { forAcademy } from '@/lib/tenant-db'
import { requireAcademyId } from '@/lib/session'
import { listLessons, type LessonListFilters } from './actions'
import { LessonsTable, type LessonRow } from './lessons-table'
import type { LessonStatusValue, LessonTypeValue } from '@/lib/validations/lesson'

export const dynamic = 'force-dynamic'

type SearchParams = {
  q?: string
  teacherId?: string
  studentId?: string
  instrument?: string
  status?: string
  lessonType?: string
  dateFrom?: string
  dateTo?: string
  sort?: string
  sortDir?: string
}

type Props = {
  searchParams: Promise<SearchParams>
}

function asStatus(v?: string): LessonStatusValue | 'ALL' | undefined {
  if (!v) return undefined
  const allowed = ['PLANNED', 'COMPLETED', 'CANCELLED', 'POSTPONED', 'NO_SHOW'] as const
  return (allowed as readonly string[]).includes(v) ? (v as LessonStatusValue) : 'ALL'
}

function asType(v?: string): LessonTypeValue | 'ALL' | undefined {
  if (!v) return undefined
  const allowed = ['PRIVATE', 'GROUP', 'TRIAL'] as const
  return (allowed as readonly string[]).includes(v) ? (v as LessonTypeValue) : 'ALL'
}

export default async function LessonsPage({ searchParams }: Props) {
  const params = await searchParams

  const filters: LessonListFilters = {
    search: params.q,
    teacherId: params.teacherId,
    studentId: params.studentId,
    instrument: params.instrument,
    status: asStatus(params.status),
    lessonType: asType(params.lessonType),
    dateFrom: params.dateFrom,
    dateTo: params.dateTo,
    sort: params.sort === 'teacher' || params.sort === 'student' ? params.sort : 'date',
    sortDir: params.sortDir === 'asc' ? 'asc' : 'desc',
  }

  try {
    const { academyId } = await requireAcademyId()
    const db = forAcademy(academyId)

    const [lessons, teachers, students] = await Promise.all([
      listLessons(filters),
      db.teacher.findMany({
        where: { isActive: true },
        orderBy: [{ firstName: 'asc' }, { lastName: 'asc' }],
        select: { id: true, firstName: true, lastName: true },
      }),
      db.student.findMany({
        where: { isActive: true },
        orderBy: [{ firstName: 'asc' }, { lastName: 'asc' }],
        select: { id: true, firstName: true, lastName: true },
      }),
    ])

    const rows: LessonRow[] = lessons.map((l) => ({
      id: l.id,
      instrument: l.instrument,
      lessonType: l.lessonType,
      lessonFee: l.lessonFee,
      room: l.room,
      notes: l.notes,
      status: l.status,
      startTime: l.startTime.toISOString(),
      endTime: l.endTime.toISOString(),
      durationMinutes: l.durationMinutes,
      student: l.student,
      teacher: l.teacher,
    }))

    return (
      <Suspense fallback={<LessonsLoadingFallback />}>
        <LessonsTable
          lessons={rows}
          teachers={teachers}
          students={students}
          filters={{
            search: params.q ?? '',
            teacherId: params.teacherId ?? '',
            studentId: params.studentId ?? '',
            instrument: params.instrument ?? '',
            status: params.status ?? '',
            lessonType: params.lessonType ?? '',
            dateFrom: params.dateFrom ?? '',
            dateTo: params.dateTo ?? '',
            sort: filters.sort ?? 'date',
            sortDir: filters.sortDir ?? 'desc',
          }}
        />
      </Suspense>
    )
  } catch (err) {
    console.error('LessonsPage failed:', err)
    return (
      <div className="flex flex-col items-center justify-center gap-3 py-24 max-w-[1400px]">
        <p className="text-sm font-medium text-foreground">Could not load lessons.</p>
        <p className="text-sm text-muted-foreground">
          Please refresh the page or try again in a moment.
        </p>
      </div>
    )
  }
}

function LessonsLoadingFallback() {
  return (
    <div className="flex flex-col gap-6 max-w-[1400px] animate-pulse">
      <div className="h-8 w-40 rounded-lg bg-muted" />
      <div className="h-9 w-full max-w-sm rounded-xl bg-muted" />
      <div className="h-64 rounded-2xl bg-muted" />
    </div>
  )
}
