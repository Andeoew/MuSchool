...
export function TeachersTable({ teachers }: { teachers: TeacherRow[] }) {
  ...
  const [modalState, setModalState] = useState<{ mode: 'create' } | { mode: 'edit'; teacher: TeacherRow } | null>(null)
  ...
  const filtered = teachers.filter((t) => {
    ...
    const matchesFilter = filter === 'all' || (filter === 'active') === t.isActive
  ...
  function handleResetPassword(teacher: TeacherRow) {
    setOpenMenuId(null)
    ...
    <div className="flex flex-col gap-6 max-w-[1400px]">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            {teachers.length} staff &bull; {teachers.filter((t) => t.isActive).length} active
          </p>
        </div>
        <button
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/50">
                {['Teacher', 'Subject', 'Joined', 'Login', 'Status', ''].map((h) => (
                  <th
                    key={h || 'actions'}
              ...
              {filtered.map((teacher, i) => (
                <tr
                  ...
                  className={cn(
                    ...
                    deletingId === teacher.id && 'opacity-50 pointer-events-none'
                  )}
                >
                  <td className="px-5 py-3.5">
                    <a href={`/dashboard/teachers/${teacher.id}`} className="flex items-center gap-3">
                      <div
                        className={cn(
                  ...
                  <td className="px-5 py-3.5 text-[13px] text-muted-foreground whitespace-nowrap">{teacher.hiredAt}</td>
                  <td className="px-5 py-3.5 text-[13px] text-muted-foreground">
                    {teacher.hasLogin ? 'Yes' : 'No'}
                  </td>
                  <td className="px-5 py-3.5">
                    <span
                      className={cn(
                    ...
                    >
                      <span className={cn('w-1.5 h-1.5 rounded-full', teacher.isActive ? 'bg-emerald-500' : 'bg-muted-foreground')} />
                  ...
                  <td className="px-5 py-3.5 relative">
                    ...
                    {openMenuId === teacher.id && (
                      <>
                        ...
                        <div className="absolute right-5 top-10 z-20 w-44 rounded-xl border border-border bg-card shadow-lg overflow-hidden">
                          <button
                            onClick={() => {
          ...
          <div className="py-16 text-center">
            {teachers.length === 0 ? (
              <div className="flex flex-col items-center gap-3 px-6">
                <p className="text-sm font-medium text-foreground">No teachers yet</p>
                <p className="text-sm text-muted-foreground max-w-sm">
                  Add your first teacher to start assigning lessons.
                </p>
                <button
                  onClick={() => setModalState({ mode: 'create' })}
      ...
      {modalState && (
        <TeacherFormModal
          ...
          teacher={modalState.mode === 'edit' ? modalState.teacher : undefined}
